...
export default class ParticleCanvas extends React.PureComponent<IProps, IState> {
    ...
    private setupNewCanvas = (canvas: HTMLCanvasElement) => {
        ...
        const backgroundColor = { r: 0.120, g: 0, b: 0.20 };
        ...
        for (let edgeIndex = 0; edgeIndex < this.edgeCount; edgeIndex++) {
            ...
            this.textureData.setVec2(VERTEX_ROW, edgeIndex, convertBezierPoints(from, from), convertBezierPoints(to, to));
            // random variation of the particles
            this.textureData.setValue(VARIATION_ROW, edgeIndex, variationMin, variationMax, variationMax - variationMin, 0.0);
            ...
            // bezier
            this.textureData.setVec2(BEZIER_ROW, edgeIndex, convertBezierPoints(from, from), convertBezierPoints(to, to));
        ...
        return (
            <div key="test" style={{ backgroundColor: "#002020" }}>
                <div style={{ color: "white", padding: 5 }}>
                    <input type="range" onChange={this.timeChange} min={0} max={4000} defaultValue={this.state.now.toString()}  style={{ width: width * 0.5, marginLeft:100, height: 50, display: "inline-block" }} />
