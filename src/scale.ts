...
/** Scale stuff from a virtual canvas */
export class Scale {
	...
	public getTransform() {
		return {
			x: this.offsetScreen.x + this.topLeftMarginVirtual.width,
	...
	public getTranslate() {
		return `translate(${this.offsetScreen.x + this.topLeftMarginVirtual.width},${this.offsetScreen.y +
			this.topLeftMarginVirtual.height})`;
	}
	...
	private updateScale() {
		...
		this.toScreenScale = {
			x: this.screenSize.width / this.virtualSize.width,
			y: this.screenSize.height / this.virtualSize.height
		};
